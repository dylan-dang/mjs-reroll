...
import { getRooms, isInGame, setAutoCallWin, isInRank, getRoomName } from "./api";
import { getDebugString } from "./logging";
import { toggleRun, setData } from "./main";
import { DEBUG_BUTTON, MODE, AIMODE, ROOM, AUTORUN, AIMODE_NAME, showingStrategy } from "./parameters";
...
export function showDebugString() {
...
export function hideButtonClick() {
	guiDiv.style.display = "none";
}

export function autorunCheckboxClick() {
...
export function refreshRoomSelection() {
...
export function showCrtActionMsg(msg) {
	if (!showingStrategy) {
...
export function showCrtStrategyMsg(msg) {
	showingStrategy = true;
	currentActionOutput.value = msg;
}

export function clearCrtStrategyMsg() {
