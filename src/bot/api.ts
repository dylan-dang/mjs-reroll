...
import { getTileDanger } from "./ai_defense";
import type { showCrtStrategyMsg } from "./gui";
import { log, getTileName } from "./logging";
import { setData } from "./main";
import { ROOM, MODE, AIMODE, ownHand, CHANGE_RECOMMEND_TILE_COLOR, calls, functionsExtended, riichiTiles, visibleTiles, playerDiscardSafetyList } from "./parameters";
import { getCorrectPlayerNumber, getNumberOfPlayers, getTileDoraValue, getCallNameByType, getTileEmojiByName } from "./utils";


export function preventAFK() {
...
export function hasFinishedMainLobbyLoading() {
	if (typeof GameMgr == 'undefined') {
		return false;
	}
...
export function getNumberOfKitaOfPlayer(player) {
	player = getCorrectPlayerNumber(player);
...
export function localPosition2Seat(player) {
	player = getCorrectPlayerNumber(player);
	return view.DesktopMgr.Inst.localPosition2Seat(player);
}

export function seat2LocalPosition(playerSeat) {
...
export function getSeatWind(player) {
...
export function getRoundWind() {
	return view.DesktopMgr.Inst.index_change + 1;
}

export function setAutoCallWin(win) {
	if (!isInGame())
		return;
...
export function getTileForCall() {
...
export function makeCallWithOption(type, option) {
	if (MODE === AIMODE.AUTO) {
...
export function declineCall(operation) {
	if (MODE === AIMODE.AUTO) {
		try {
...
export function sendRiichiCall(tile, moqie) {
	if (MODE === AIMODE.AUTO) {
...
export function sendAbortiveDrawCall() {
	if (MODE === AIMODE.AUTO) {
...
export function callDiscard(tileNumber) {
	if (MODE === AIMODE.AUTO) {
		try {
...
export function getPlayerLinkState(player) {
	player = getCorrectPlayerNumber(player);
	return view.DesktopMgr.player_link_state[localPosition2Seat(player)];
}

export function getNumberOfTilesInHand(player) {
	player = getCorrectPlayerNumber(player);
...
export function doesPlayerExist(player) {
...
export function hasPlayerHandChanged(player) {
	var player_correct = getCorrectPlayerNumber(player);
...
export function rememberPlayerHand(player) {
	var player_correct = getCorrectPlayerNumber(player);
...
export function getOverallTimeLeft() {
	try {
...
export function getLastTurnTimeLeft() {
	try {
...
export function extendMJSoulFunctions() {
...
export function trackDiscardTiles() {
