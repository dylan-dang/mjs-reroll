...
import { getSeatWind, getRoundWind, isPlayerRiichi, getNumberOfKitaOfPlayer, getNumberOfTilesInHand, localPosition2Seat, getPlayerLinkState, getCurrentRoom, hasPlayerHandChanged, seat2LocalPosition, getCurrentPlayer, rememberPlayerHand } from "./api";
import { riichiTiles, discards, tilesLeft, calls, availableTiles, playerDiscardSafetyList, roundWind, ownHand, SAKIGIRI, dora } from "./parameters";
import { getNumberOfPlayers, getTileDoraValue, getNumberOfTilesAvailable, getCorrectPlayerNumber, getNumberOfDoras, calculateScore, getUradoraChance, isSameTile, getSequences, getNumberOfTilesInTileArray, getHigherTileIndex } from "./utils";

import type { Tile } from "./parameters";

var totalPossibleWaits: { turn?: number, totalWaits?: number[] } = {};
...
export function getTileDanger(tile: Tile, playerPerspective = 0) {
...
export function getTileDangerForPlayer(tile: Tile, player: number, playerPerspective = 0) {
	var danger = 0;
...
export function getDealInChanceForTileAndPlayer(player: number, tile: Tile, playerPerspective = 0) {
	var total = 0;
...
export function getTotalPossibleWaits(player: number) {
	var total = 0;
...
export function getExpectedDealInValue(player: number) {
...
export function getExpectedHandValue(player: number) {
...
export function getExpectedDoraInHand(player: number) {
	var uradora = 0;
...
export function getCurrentDangerLevel(forPlayer = 0) { //Most Dangerous Player counts extra
	var i = 1;
	var j = 2;
	var k = 3;
	if (forPlayer == 1) {
...
export function getMostRecentDiscardDanger(tile: Tile, player: number, includeOthers: boolean) {
	var danger = 99;
...
export function getLastTileInDiscard(player: number, tile: Tile) {
...
export function wasTileCalledFromOtherPlayers(player: number, tile: Tile) {
	for (var i = 0; i < getNumberOfPlayers(); i++) {
...
export function isPlayerTenpai(player: number) {
	var numberOfCalls = Math.floor(calls[player].length / 3);
...
export function isPlayerPushing(player: number) {
...
export function hasYaku(player: number) {
...
export function getConfidenceInYakuPrediction(player: number) {
	var confidence = Math.pow(Math.floor(calls[player].length / 3), 2) / 10;
	if (confidence > 1) {
		confidence = 1;
	}
	return confidence;
...
export function isDoingHonitsu(player: number, type: number) {
	if (Math.floor(calls[player].length) == 0 || calls[player].some(tile => tile.type != type && tile.type != 3)) { //Calls of different type -> false
		return 0;
	}
	if (Math.floor(calls[player].length / 3) == 4) {
		return 1;
	}
	...
	var confidence = (Math.pow(Math.floor(calls[player].length / 3), 2) / 10) - percentageOfDiscards + 0.1;
	if (confidence > 1) {
		confidence = 1;
	}
	return confidence;
...
export function isDoingToiToi(player: number) {
	if (Math.floor(calls[player].length) > 0 && getSequences(calls[player]).length == 0) { //Only triplets called
...
export function isDoingTanyao(player: number) {
	if (Math.floor(calls[player].length) > 0 && calls[player].filter(tile => tile.type == 3 || tile.index == 1 || tile.index == 9).length == 0 &&
...
export function isDoingYakuhai(player: number) {
	var yakuhai = Math.floor(calls[player].filter(tile => tile.type == 3 && (tile.index > 4 || tile.index == getSeatWind(player) || tile.index == roundWind)).length / 3);
	yakuhai += Math.floor(calls[player].filter(tile => tile.type == 3 && tile.index == getSeatWind(player) && tile.index == roundWind).length / 3);
	return yakuhai;
}
...
export function getWaitScoreForTileAndPlayer(player: number, tile: Tile, includeOthers: boolean, useKnowledgeOfOwnHand = true) {
...
export function getFuritenValue(player: number, tile: Tile, includeOthers: boolean) {
...
export function updateDiscardedTilesSafety() {
...
export function initialDiscardedTilesSafety() {
	for (var k = 1; k < getNumberOfPlayers(); k++) { //For all other players
		for (var i = 0; i < getNumberOfPlayers(); i++) { //For all discard ponds
			for (var j = 0; j < discards[i].length; j++) { //For every tile in it
				if (typeof (discards[i][j].numberOfPlayerHandChanges) == 'undefined') {
...
export function getSakigiriValue(hand: Tile[], tile: Tile) {
	var sakigiri = 0;
...
export function isSafeTile(player: number, tile: Tile) {
...
export function isTileCloseToOtherTile(tile: Tile, otherTile: Tile) {
...
export function isTileCloseToDora(tile: Tile) {
	for (let d of dora) {
