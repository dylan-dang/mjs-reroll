...
import { isPlayerTenpai, getExpectedHandValue, updateDiscardedTilesSafety, initialDiscardedTilesSafety } from "./ai_defense";
import { determineStrategy, callAnkan, callShouminkan, callTsumo, callRon, callKita, callAbortiveDraw, callDaiminkan } from "./ai_offense";
import { preventAFK, isInGame, hasFinishedMainLobbyLoading, isDisconnect, getOperationList, getTilesLeft, getOperations, getOverallTimeLeft, getLastTurnTimeLeft, getDora, getPlayerHand, getDiscardsOfPlayer, getCallsOfPlayer, localPosition2Seat, setAutoCallWin, extendMJSoulFunctions, getSeatWind, getRoundWind, searchForGame, isEndscreenShown } from "./api";
import { initGui, toggleGui, clearCrtStrategyMsg, refreshRoomSelection, showCrtActionMsg } from "./gui";
import { log, getDebugString } from "./logging";
import { AUTORUN, run, AIMODE_NAME, MODE, errorCounter, lastTilesLeft, AIMODE, threadIsRunning, isConsideringCall, timeSave, dora, ownHand, MARK_TSUMOGIRI, discards, calls, isClosed, tilesLeft, strategy, STRATEGIES, strategyAllowsCalls, riichiTiles, playerDiscardSafetyList, seatWind, roundWind, ROOM } from "./parameters";
import { isDebug, getNumberOfPlayers, updateAvailableTiles } from "./utils";
...
export function waitForMainLobbyLoad() {
...
var oldOps = []
export function recordPlayerOps() {
	oldOps = []
...
export function checkPlayerOpChanged() {
	let ops = getOperationList();
...
export function setData(mainUpdate = true) {
