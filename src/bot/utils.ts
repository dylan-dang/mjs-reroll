...
import { getDealInChanceForTileAndPlayer, getCurrentDangerLevel, getTileDanger } from "./ai_defense";
import { doesPlayerExist, localPosition2Seat, getOperationList, getOperations, getSeatWind, getPlayerScore, isEastRound, getRound, getRoundWind } from "./api";
import { log, getTileName, getTileFromString } from "./logging";
import { PERFORMANCE_MODE, timeSave, visibleTiles, calls, discards, dora, ownHand, availableTiles, seatWind, roundWind, isClosed, tilesLeft, strategy, STRATEGIES, SAFETY, strategyAllowsCalls, RIICHI, tileEmojiList } from "./parameters";
...
export function getCorrectPlayerNumber(player) {
...
export function isSameTile(tile1, tile2, checkDora = false) {
...
export function getNumberOfDoras(tiles) {
	var dr = 0;
...
//Pairs in tiles
export function getPairs(tiles) {
...
export function getPairsAsArray(tiles) {
...
export function getDoubles(tiles) {
...
export function getTriplesAndPairs(tiles) {
...
export function getTriples(tiles) {
	var sequences = getSequences(tiles);
	var triplets = getTriplets(tiles);
...
export function getTriplets(tiles) {
	var sortedTiles = sortTiles(tiles);
...
export function getTripletsAsArray(tiles) {
	var triplets = getTriplets(tiles);
	var tripletsList = [];
...
export function getBestSequenceCombination(inputHand) {
...
export function pushTileAndCheckDora(tiles, arrayToPush, tile) {
...
export function getBestCombinationOfTiles(inputTiles, possibleCombinations, chosenCombinations) {
...
export function getSequences(tiles) {
	var sortedTiles = sortTiles(tiles);
	var sequences = [];
...
export function removeTilesFromTileArray(inputTiles, tiles) {
...
//Sort tiles
export function sortTiles(inputTiles) {
...
export function getNumberOfTilesAvailable(index, type) {
...
export function isTileFuriten(index, type) {
...
export function getNumberOfNonFuritenTilesAvailable(index, type) {
...
export function getTileDoraValue(tile) {
	var dr = 0;
...
export function getHigherTileIndex(tile) {
	if (tile.type == 3) {
		if (tile.index == 4) {
			return 1;
		}
...
export function getHandWithCalls(inputHand) {
	return inputHand.concat(calls[0]);
}

//Adds a tile if not in array
export function pushTileIfNotExists(tiles, index, type) {
...
export function getUradoraChance() {
	if (getNumberOfPlayers() == 4) {
...
export function getUsefulTilesForTriple(tileArray) {
	var tiles = [];
...
export function getUsefulTilesForDouble(tileArray) {
	var tiles = [];
	for (let tile of tileArray) {
		pushTileIfNotExists(tiles, tile.index, tile.type);
		if (tile.type == 3) {
...
export function getAllTerminalHonorFromHand(hand) {
...
export function getWaitQuality(tile) {
...
export function calculateShanten(triples, pairs, doubles) {
...
export function calculateScore(player, han, fu = 30) {
...
export function calculateFu(triples, openTiles, pair, waitTiles, winningTile, ron = true) {
	var fu = 20;
...
export function isValueTile(tile) {
...
export function getFoldThreshold(tilePrio, hand) {
...
export function shouldFold(tile, highestPrio = false) {
...
export function shouldRiichi(tilePrio) {
...
export function getDistanceToFirst() {
	if (getNumberOfPlayers() == 3) {
...
export function getDistanceToLast() {
	if (getNumberOfPlayers() == 3) {
...
export function getDistanceToPlayer(player) {
...
export function isWinningHand(numberOfTriples, numberOfPairs) {
	if (strategy == STRATEGIES.CHIITOITSU) {
...
export function getCallNameByType(type) {
	switch (type) {
...
export function getTileEmojiByName(name) {
