...
import type { getCurrentDangerLevel, getTileDanger, getSakigiriValue, isSafeTile, getWaitScoreForTileAndPlayer } from "./ai_defense";
import { getTileForCall, declineCall, makeCallWithOption, getOperations, isPlayerRiichi, makeCall, sendKitaCall, sendAbortiveDrawCall, getOperationList, sendRiichiCall, callDiscard, getNumberOfKitaOfPlayer } from "./api";
import { log, getTileName, getTileFromString, printTilePriority, printHand, getTilesFromString } from "./logging";
import { strategy, STRATEGIES, ownHand, CHIITOITSU, isClosed, strategyAllowsCalls, tilesLeft, calls, CALL_PON_CHI, seatWind, CALL_KAN, discards, dora, PERFORMANCE_MODE, timeSave, availableTiles, EFFICIENCY, SAFETY, THIRTEEN_ORPHANS, KEEP_SAFETILE } from "./parameters";
import { getTriples, getHandWithCalls, getPairsAsArray, removeTilesFromTileArray, getTriplesAndPairs, getDoubles, calculateShanten, getNumberOfDoras, isSameTile, isValueTile, getNumberOfTilesAvailable, getWallSize, getNumberOfTilesInTileArray, shouldRiichi, getFoldThreshold, isDebug, getNumberOfPlayers, getTileDoraValue, getUsefulTilesForDouble, getUsefulTilesForTriple, isWinningHand, isTileFuriten, getWaitQuality, calculateFu, calculateScore, getUradoraChance, isLastGame, getDistanceToFirst, getNumberOfNonFuritenTilesAvailable, getAllTerminalHonorFromHand, canRiichi, shouldFold } from "./utils";
import type { getYaku } from "./yaku";

//Look at Hand etc. and decide for a strategy.
export function determineStrategy() {

	if (strategy != STRATEGIES.FOLD) {
		var handTriples = Math.floor(getTriples(getHandWithCalls(ownHand)).length / 3);
...
async function callTriple(combinations, operation) {
	...
	for (var i = 0; i < combinations.length; i++) {
		...
		var shanten = calculateShanten(Math.floor(newHandTriples.triples.length / 3), Math.floor(newHandTriples.pairs.length / 2), Math.floor(doubles.length / 2));
...
export function callShouminkan() {
	callKan(getOperations().add_gang, getTileForCall());
}

//Closed Kan
export function callAnkan(combination) {
...
export function callKan(operation, tileForCall) {
	log("Consider Kan.");
...
export function callRon() {
	makeCall(getOperations().rong);
}

export function callTsumo() {
	makeCall(getOperations().zimo);
}

export function callKita() { // 3 player only
...
export function callAbortiveDraw() { // Kyuushu Kyuuhai, 9 Honors or Terminals in starting Hand
...
export function callRiichi(tiles) {
...
export function discardFold(tiles) {
...
export function discardTile(tile) {
	if (!tile.valid) {
		return;
	}
...
*/
export function getHandValues(hand, discardedTile) {
	var shanten = 8; //No check for Chiitoitsu in this function, so this is maximum

	var callTriples = Math.floor(getTriples(calls[0]).length / 3);
	...
	var baseShanten = calculateShanten(Math.floor(triples.length / 3) + callTriples, Math.floor(pairs.length / 2), Math.floor(doubles.length / 2));
	...
	if (typeof discardedTile != 'undefined') { //When deciding whether to call for a tile there is no discarded tile in the evaluation
		...
		var originalShanten = calculateShanten(Math.floor(originalTriples.length / 3) + callTriples, Math.floor(originalPairs.length / 2), Math.floor(originalDoubles.length / 2));
		hand.pop();
	}
	else {
	...
	for (let tileCombination of tileCombinations) {
		...
		var winning = isWinningHand(Math.floor((triplesAndPairs2.triples.length / 3)) + callTriples, triplesAndPairs2.pairs.length / 2);
		if (winning) {
	...
	for (let tileCombination of tileCombinations) { //Now again go through all the first tiles, but also the second tiles
		...
		for (let tile2Data of tileCombination.tiles2) {
			...
			var winning2 = isWinningHand(Math.floor((triplesAndPairs3.triples.length / 3)) + callTriples, triplesAndPairs3.pairs.length / 2);
	...
	for (let tileCombination of tileCombinations) {
		...
		if (tileCombination.winning && !tile1Furiten) { //Hand is winning: Add the values of the hand for most possible ways to draw this:
			...
			if (tile1Furiten) {
				...
				thisShanten = (calculateShanten(Math.floor(triples2.length / 3) + callTriples, Math.floor(pairs2.length / 2), Math.floor(doubles2.length / 2)) - baseShanten);
			}
		}
		...
		for (let tile2Data of tileCombination.tiles2) {//Look at second tiles if not already winning
			...
			var thisShanten = 8;
			var winning = isWinningHand(Math.floor((triples3.length / 3)) + callTriples, pairs3.length / 2);
			...
			if (winning && !tile2Furiten) { //If this tile combination wins in 2 turns: calculate shape etc.
				...
				if (winning && (tile2Furiten || (!isClosed && thisYaku.open < 1))) { //Furiten/No Yaku: We are 0 shanten
					...
					thisShanten = calculateShanten(Math.floor(triples3.length / 3) + callTriples, Math.floor(numberOfPairs / 2), Math.floor(numberOfDoubles / 2)) - baseShanten;
...
export function calculateTilePriority(efficiency, expectedScore, danger) {
...
export function chiitoitsuPriorities() {
...
export function thirteenOrphansPriorities() {
...
export function canDoThirteenOrphans() {
...
export function getMissingTilesForThirteenOrphans(uniqueTerminalHonors) {
...
export function sortOutUnsafeTiles(tiles) {
	for (let tile of tiles) {
		if (tile == tiles[0]) {
...
export function keepSafetile(tiles) {
...
export function getDiscardTile(tiles) {
